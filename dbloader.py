...
class EntityLoader():
    ...
    def __init__(self, filename, reader):
        ...
        try:
            self._df = pd.read_csv(
                self._filename[:-4] + '.cache.txt', sep=',', encoding='utf8')
        except:
        ...
        with self._driver.session() as session:
            try:
                response = session.write_transaction(
                    EntityLoader.call, arg1, rel, arg2)
    ...
    def cache(self):
        self._df.to_csv(
            self._filename[:-4] + '.cache.txt', sep=',', encoding='utf8', index=False)
...
def yagoReader(filename):
    ...
    with open(filename, encoding="utf8") as f:
        for line in f:
            arr.append([el.replace('<', '').replace('>', '').lower()
...
def findMatches(entity1, entity2):
    ...
    # indicies =
    merged = pd.merge(entity1, entity2, left_on=[
                      'e1', 'e2'], right_on=['e1', 'e2'])
...
class KillClassifier():
    ...
    def predict(self, x):
        ...
        for i, k in enumerate(x):
            ...
            temp = self.matrix_cosine(np.broadcast_to(
                x[i], self.train_x.shape), self.train_x)
...
def convertToEmbeddings(words, gensim):
    ...
    for i, val in enumerate(words):
        ret.append(
            np.mean(np.array([lookup(gensim, k)
                              for k in str(val).split('_')]),
                    axis=0))
    ret = np.array(ret)
...
def classifyEntities(wiki, yago):
    # Maps the wiki classifications to yago ones, or declares them
    #  'out of class'
    gensim = loadGensim()
    ...
    # make the word vecs of the unique yago classes the training
    # x and the actual words the labels
